...
def run_mcmc(data_x, data_y, data_v, data_flux,
    ...
    burn_backend = HDFBackend('_burnin_temp.h5')
    ...
    try:
        ...
        # Re-create sampler with production backend
        sampler = emcee.EnsembleSampler(
            n_walkers, ndim, log_probability,
            args=(data_x, data_y, data_v, data_flux),
            moves=moves,
        ...
        # Read thinned chain from disk (no full chain in memory)
        ...
        if pool is not None:
            pool.close()
            pool.join()
        # Clean up burn-in temp file
        burn_backend_file = '_burnin_temp.h5'
